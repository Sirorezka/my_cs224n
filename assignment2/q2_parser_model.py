...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        concatenates those vectors:
            ...
            - Concatenates
             the embeddings by reshaping the embeddings tensor to shape
        ...
        embed = tf.Variable(self.pretrained_embeddings, 'pretr_embed')
        idxs = tf.reshape(self.input_placeholder,shape = [-1])
        embed = tf.gather(embed,indices = tf.cast(idxs,tf.int32))
    ...
    def add_prediction_op(self):
        ...
        w1 = tf.Variable(xavier_initializer([self.config.n_features*self.config.embed_size,
                                            self.config.hidden_size]), name = 'W1')  
        u1 = tf.Variable(xavier_initializer([self.config.hidden_size,
                                            self.config.n_classes]), name = 'U1')
        b1 = tf.Variable(tf.zeros(shape=[1,self.config.hidden_size],name = 'b1'))
        b2 = tf.Variable(tf.zeros(shape=[1,self.config.n_classes],name = 'b2'))

        aa = tf.matmul(x,w1)
        h = tf.nn.relu(aa+b1)      
        h_drop = tf.nn.dropout(h,keep_prob = 1-self.dropout_placeholder)         
        pred = tf.matmul(h_drop,u1) + b2
        ### END YOUR CODE
        return pred
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        entropy = tf.nn.softmax_cross_entropy_with_logits_v2(labels = self.labels_placeholder,
                                                            logits = pred)
